...
public class Quiz
{
    ...
    {
        int chapter = rand.nextInt(questions.length);
        int quesiton = rand.nextInt(questions[chapter].length);
    ...
    private static void giveQuiz(int numOfQuestions)
    {
        ...
        for (int i = 0; i < numOfQuestions; i++)
        {
            ...
            else
            {
                ...
                System.out.println("Please see chapter " + (data[0] + 5) + ", item " + (data[1] + 1));
                System.out.println("Current score: " + correct + " / " + total + " = " + score(correct, wrong, total) + "%");
            }
        }
